...
export function renderWindDailyText(hours) {
  ...
  $("ds-summary").textContent = `${dayLabel(days[0].date)} to ${dayLabel(days.at(-1).date)}. Strongest hourly wind: ${top.ws.toFixed(1)} km/h from ${sectorName(top.dir)} on ${dayLabel(top.date)}.`;
  $("ds-note").textContent =
  ...
  for (const h of ["Day", "Most common direction", "Mean speed (km/h)", "Peak hourly speed (km/h)", "Hours shown"]) {
