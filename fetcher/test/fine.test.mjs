...
const row = (ts, o = {}) => [ts, 0, 1, 2, 90, 3, o.p ?? 1000, 27, 80, 0, 1, o.solar ?? 200, o.rain ?? 0, 0, 0, 0, 2.6, 1];
const T = Date.UTC(2026, 8, 21, 20, 0, 0) / 1000; // 14:00 local, on a bucket boundary

test("groups minutes into 10-minute buckets: rain sums, solar and pressure average", () => {
  const rows = [row(T, { rain: 0.2, solar: 100, p: 1000 }), row(T + 540, { rain: 0.3, solar: 300, p: 1001 }), row(T + 600, { rain: 1 })];
  const b = aggregateFine(rows);
  assert.equal(b.size, 2);
  assert.deepEqual(b.get(T), { rain: 0.5, solar: 200, p: 1000.5, n: 2 });
